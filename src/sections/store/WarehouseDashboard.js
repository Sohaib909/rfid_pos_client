...
const WarehouseDashboard = () => {
  ...
  const handleTransfer = async (e) => {
    ...
    try {
      ...
      setTransfers(transRes.data.filter(t => t.fromStore._id === currentStore._id));
    } catch (err) {
  ...
  return (
    <Container>
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
        <Typography variant="h4">Warehouse Dashboard - {currentStore?.name}</Typography>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        ...
        <Box display="flex" flexDirection="column" gap={1}>
          {products.length === 0 && <Typography>No products in this warehouse.</Typography>}
      ...
      <Paper sx={{ p: 2 }}>
        <Box display="flex" alignItems="center" justifyContent="space-between" mb={1}>
          ...
          <CSVLink data={csvData} filename={`warehouse-transfers-${currentStore?.name}.csv`}>
            <Button variant="outlined" color="primary">Export Transfers</Button>
          </CSVLink>
        </Box>
        <Box display="flex" flexDirection="column" gap={1}>
          {transfers.length === 0 && <Typography>No transfers from this warehouse yet.</Typography>}
